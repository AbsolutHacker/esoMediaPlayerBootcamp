...
public final class MediaBrowserImpl implements MediaBrowser {
  ...
  private static final MediaBrowserImpl mediaBrowser = new MediaBrowserImpl();
  ...
  public Observable<List<Album>> searchAlbum(String name, Set<Audio.Connection> searchScope) {
    ...
    Observable<List<Album>> searchedAlbums$ =
        albums$.map(
            albums ->
                albums
                    .stream()
  ...
  public Observable<List<Track>> searchTrack(String name, Set<Audio.Connection> searchScope) {
    ...
    @SuppressWarnings("UnnecessaryLocalVariable")
    Observable<List<Track>> searchedTracks$ =
        tracks$.map(
            tracks ->
                tracks
                    .stream()
  ...
  public Observable<List<Track>> getAlbumTracks(Album album) {
    ...
    @SuppressWarnings("UnnecessaryLocalVariable")
    Observable<List<Track>> albumTracks$ =
        tracks$.map(
            tracks ->
                tracks
                    .stream()
  ...
  private Observable<List<Track>> getAllTracksInScope(Set<Audio.Connection> searchScope) {
    ...
    Observable<List<Track>> radioTrackList$ =
        radioTrack$.map(
            track -> {
    ...
    // concat all different sources together. Here the default information what came from where is
    // lost
  ...
  private Observable<List<Album>> getAlbumsFromTracks(Observable<List<Track>> tracks$) {
    ...
    Observable<List<Integer>> albumIds$ =
        tracks$.map(
            tracks ->
                tracks.stream().map(Track::getAlbumId).distinct().collect(Collectors.toList()));

    // map each Integer to it's album
    @SuppressWarnings("UnnecessaryLocalVariable")
    Observable<List<Album>> gottenAlbums =
        albumIds$.flatMapSingle(
            albumIds -> { // flatMapSINGLE
              List<Single<Album>> singleList =
                  albumIds
                      .stream()
              ...
              return Single.zip(
                  singleList,
                  array -> { // zip that Single<Album> back to Album
