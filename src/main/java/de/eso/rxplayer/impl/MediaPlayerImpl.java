                  ...
                  .flatMapCompletable(
                      (verifyPlayState) -> {
                        if (verifyPlayState) {
                          ...
                          return es.getAudio().fadeOut(currentSource).andThen(player.pause());
                        } else {
    ...
    if (player == null) {
      ...
      switch (currentState) {
        ...
        case STARTED:
          ...
          return pauseInner().doOnComplete(() -> es.getAudio().stop(currentSource));
        case STOPPING:
                ...
                return player
                    ...
                    .flatMapCompletable(
                        trackid -> {
                          ...
                          currentTrackId++;
                          return pauseInner().andThen(playInner(trackid + 1));
                        })
        ...
        .subscribe(
            state -> {
              ...
              switch (state) {
                case STARTED:
                  System.out.println(
                      "manageSources:  STARTED -> " + source.name() + " -> " + "Nothing happens");
                  break;
                case STOPPED:
                  System.out.println(
                      "manageSources: STOPPED -> " + currentSource + " != " + source);
